<template>
  <div class="average-age">
    <div class="title-wrapper">
      <div class="left">
        <div class="title">数据中台用户年龄分布&平均年龄</div>
...
export default {
  ...
  setup (ctx) {
    ...
    options.value = createOption(ctx.data)
    watch(() => ctx.avgAge, (nextValue, preValue) => {
      startAge.value = preValue
    })
    watch(() => ctx.data, (next, pre) => {
      options.value = createOption(next)
    })
    return {
      startAge,
      options
