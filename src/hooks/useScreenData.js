...
export default function () {
  ...
  onMounted(() => {
    task = setInterval(() => {
      ...
      // 热门种类数据
      const _hotCategory = Object.assign({}, hotCategory.value)
      _hotCategory.data1.data1.forEach(item => {
        item = item + 10
      })
      _hotCategory.data1.data2.forEach(item => {
        item = item + 10
      })
      _hotCategory.data2.data1.forEach(item => {
        item = item + 10
      })
      _hotCategory.data2.data2.forEach(item => {
        item = item + 10
      })
      hotCategory.value = _hotCategory
      const _headerData = Object.assign({}, headerData.value)
      _headerData.headerData.value.forEach(item => {
        item.startValue = item.endVal
